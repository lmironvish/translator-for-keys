<template>
    <div class="user-webinars-table">
        ...
        />
        <!--        @searchChange="searchChange($event, activeTableIndex)"
        @clearFilters="clearFilters(activeTableIndex)"-->
        <table-block
...
export default {
    ...
    methods: {
        ...
        async fetchData() {
            try {
                this.content = await api.webinar.getWebinarsReportTable({
                    teacher: this.userId,
                    page: this.page,
                    ...this.filter
                });
...
<style src="./user-webinars-teaching-table.less" lang="less" />
